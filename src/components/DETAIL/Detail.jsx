...
const Detail = () => {
  ...
  const { item, value, increQuantity, decreQuantity, id, currencyRupiah } = useContext(QuantityContext);
  ...
  return (
    <>
      <div className="w-full h-screen sm:py-2 lg:flex lg:items-center md:mt-4 lg:mt-8">
        <div className="container mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:py-4">
            <div className="w-full md:w-3/6 lg:mx-2 mb-4 md:mb-0 flex flex-col gap-y-4">
              <div className="bg-color-gray min-w-full h-[300px] lg:h-[400px] rounded-xl flex items-center justify-center">
                <img src={item.data?.url_images} alt={item.data?.name} />
              </div>
              <NavigationSwipe />
            </div>
            <div className="w-full md:w-3/6 md:px-4 lg:mx-2">
              <div className="px-2 md:px-0">
                ...
                <div className="border-b border-black pb-4 border-opacity-30">
                  <h1 className="w-full font-bold text-2xl lg:text-3xl overflow-hidden max-h-[100px]">{item.data?.name}</h1>
                  <p className="my-2 w-full max-h-[150px] overflow-hidden">{item.data?.description}</p>
                ...
                <div className="flex gap-2 gap-x-5 items-center my-8">
                  ...
                  <div>Hanya tersisa {item.data?.quantity} Items!</div>
                </div>
                {/* Quantity */}

                {/* Buy Now or Keranjang */}
                <ButtonBuy redirectTo={redirectPayment} />
                {/* Buy Now or Keranjang */}
